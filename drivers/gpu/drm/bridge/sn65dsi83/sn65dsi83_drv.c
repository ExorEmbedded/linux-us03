    ...
    struct sn65dsi83 *sn65dsi83)
{
    ...
    sn65dsi83->brg->gpio_envdd = devm_gpiod_get(dev, "envdd", GPIOD_OUT_LOW);
    if (IS_ERR(sn65dsi83->brg->gpio_envdd)) {
        dev_err(dev, "failed to parse envdd gpio");
        return PTR_ERR(sn65dsi83->brg->gpio_envdd);
    }
